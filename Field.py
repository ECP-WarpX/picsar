...
class Field:
  ...
  def plot1d(self,fig,ax,slice1,slice2,slice_index_1,slice_index_2):
      if slice1=='x' and slice2=='y':
        index1 = int((slice_index_1-self.xmin)/self.dx)
	index2 = int((slice_index_2-self.ymin)/self.dy)
	im = ax.plot(self.z,self.f[:,index2,index1])
	ax.set_xlabel('z')
	ax.set_ylabel('field')
	ax.set_xlim([self.zmin,self.zmax])
	ax.set_title('slice at x=%f,y=%f'%(self.x[index1],self.y[index2]))
      if slice1=='x' and slice2=='z':
	index1 = int((slice_index_1-self.xmin)/self.dx)
        index2 = int((slice_index_2-self.zmin)/self.dz)
        im = ax.plot(self.y,self.f[index2,:,index1])
        ax.set_xlabel('y')
        ax.set_ylabel('field')
        ax.set_xlim([self.ymin,self.ymax])
        ax.set_title('slice at x=%f,z=%f'%(self.x[index1],self.z[index2]))

      if slice1=='y' and slice2=='z':
	index1 = int((slice_index_1-self.ymin)/self.dy)
	index2 = int((slice_index_2-self.zmin)/self.dz)
        im = ax.plot(self.x,self.f[index2,index1,:])
        ax.set_xlabel('x')
        ax.set_ylabel('field')
	ax.grid(True)
        ax.set_xlim([self.xmin,self.xmax])
        ax.set_title('slice at y=%f,z=%f'%(self.y[index1],self.z[index2]))
	#data=self.f[index2,index1,:]
	#ff=np.fft.fft(data)
	#plt.figure()
	#plt.plot(abs(ff)[np.arange(ff.size/2,ff.size)]**2)	
  #def movie(self,fig,slice,slice_pos)
  #    if slice=='x':
  #      index = int((slice_pos-self.xmin)/self.dx)
  #    if slice=='y':
  #      index = int((slice_pos-self.ymin)/self.dy)
