...
To run the executable on n MPI processes: "mpirun -np n a.out". Notice that if nprocx, nprocy and nprocz are provided in the input file, then n must be equal to nprocx x nprocy x nprocz with nprocx, nprocy, nprocz the number of processors along x,y,z directions. Otherwise, if nprocx, nprocy and nprocz are not defined, the code performs automatic CPU split in each direction. 
