...
const double double_tolerance = 2.0e-11;

//Tolerance for single precision calculations
const float float_tolerance = 2.0e-4;
...
BOOST_AUTO_TEST_CASE( picsar_generic_2d_table_setall)
{
    ...
    const auto val = 42.0;
    const auto vec = std::vector<double>(
        static_cast<std::size_t>(gtab_2d.get_how_many_x())*
        static_cast<std::size_t>(gtab_2d.get_how_many_y()),
        val);

    gtab_2d.set_all_vals(vec);

    const auto x0 = gtab_2d.get_x_min();
    const auto x1 = gtab_2d.get_x_max();
    const auto x2 = 0.376*(x1-x0) + x0;

    const auto y0 = gtab_2d.get_y_min();
    const auto y1 = gtab_2d.get_y_max();
    const auto y2 = 0.688*(y1-y0) + y0;
