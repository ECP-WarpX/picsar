...
INTEGER(idp), PARAMETER ::  ns_max=10
INTEGER(idp) :: nsolvers=0
...
! SUBROUTINE for associating fortran block pointers to python numpy arrays 
! (Ex, Ey etc.)
SUBROUTINE point_to_vector_block_p2f(ain,n1,n2,n3,iv,mat_index,old)
USE matrix_coefficients 
IMPLICIT NONE 
INTEGER(8), INTENT(IN) :: n1,n2,n3, mat_index, iv
...
nsolvers=nsolvers+1
ALLOCATE(cc_mat(nsolvers)%block_matrix2d(nvar,nvar), &
         vold(nsolvers)%block_vector(nvar), &
         vnew(nsolvers)%block_vector(nvar))
cc_mat(nsolvers)%nblocks=nvar 
vold(nsolvers)%nblocks=nvar 
vnew(nsolvers)%nblocks=nvar 
...
IF (nc1*nc2*nc3 .EQ. 1) THEN 
    IF (coeff1(1,1,1) .EQ. 0.) THEN 
       RETURN 
    ELSE IF (coeff1(1,1,1) .EQ. 1.) THEN 
