...
!=================================================================================
  ...
  REAL(num), DIMENSION(-nxguard:nx+nxguard,-nyguard:ny+nyguard,-nzguard:nz+nzguard) :: bxg,byg,bzg
  REAL(num)                            :: xmin,ymin,zmin,dx,dy,dz
  INTEGER(idp)                         :: ip, j, k, l
  INTEGER(idp)                         :: ixmin, ixmax, iymin, iymax, izmin, izmax, &
...
!=================================================================================                                      
  ...
  INTEGER(idp)                         :: ip, j, k, l, ixmin, ixmax, iymin, iymax, izmin, izmax, &
              ixmin0, ixmax0, iymin0, iymax0, izmin0, izmax0
  INTEGER(idp)                         :: jj, kk, ll, j0, k0, l0
  REAL(num) :: dxi, dyi, dzi, x, y, z, xint, yint, zint, &
              xintsq,oxint,yintsq,oyint,zintsq,ozint,oxintsq,oyintsq,ozintsq
  REAL(num), DIMENSION(-1:1) :: sx,sx0
  REAL(num), DIMENSION(-1:1) :: sy,sy0
...
!DIR$ DISTRIBUTE POINT
    ...
    DO nn=ip,ip+MIN(lvect,np-ip+1)-1
      ...
      ex(nn) = ex(nn) + sx0(0)*sy(1)*sz(-1)*exg(j0,k+1,l-1)
      ex(nn) = ex(nn) + sx0(1)*sy(1)*sz(-1)*exg(j0+1,k+1,l-1)
      ex(nn) = ex(nn) + sx0(-1)*sy(2)*sz(-1)*exg(j0-1,k+2,l-1)
      ex(nn) = ex(nn) + sx0(0)*sy(2)*sz(-1)*exg(j0,k+2,l-1)
      ex(nn) = ex(nn) + sx0(1)*sy(2)*sz(-1)*exg(j0+1,k+2,l-1)
      ex(nn) = ex(nn) + sx0(-1)*sy(-1)*sz(0)*exg(j0-1,k-1,l)
      ex(nn) = ex(nn) + sx0(0)*sy(-1)*sz(0)*exg(j0,k-1,l)
      ex(nn) = ex(nn) + sx0(1)*sy(-1)*sz(0)*exg(j0+1,k-1,l)
      ...
      ex(nn) = ex(nn) + sx0(0)*sy(1)*sz(0)*exg(j0,k+1,l)
      ex(nn) = ex(nn) + sx0(1)*sy(1)*sz(0)*exg(j0+1,k+1,l)
      ex(nn) = ex(nn) + sx0(-1)*sy(2)*sz(0)*exg(j0-1,k+2,l)
      ex(nn) = ex(nn) + sx0(0)*sy(2)*sz(0)*exg(j0,k+2,l)
      ex(nn) = ex(nn) + sx0(1)*sy(2)*sz(0)*exg(j0+1,k+2,l)
      ex(nn) = ex(nn) + sx0(-1)*sy(-1)*sz(1)*exg(j0-1,k-1,l+1)
      ex(nn) = ex(nn) + sx0(0)*sy(-1)*sz(1)*exg(j0,k-1,l+1)
      ex(nn) = ex(nn) + sx0(1)*sy(-1)*sz(1)*exg(j0+1,k-1,l+1)
      ...
      ex(nn) = ex(nn) + sx0(0)*sy(2)*sz(1)*exg(j0,k+2,l+1)
      ex(nn) = ex(nn) + sx0(1)*sy(2)*sz(1)*exg(j0+1,k+2,l+1)
      ex(nn) = ex(nn) + sx0(-1)*sy(-1)*sz(2)*exg(j0-1,k-1,l+2)
      ex(nn) = ex(nn) + sx0(0)*sy(-1)*sz(2)*exg(j0,k-1,l+2)
      ex(nn) = ex(nn) + sx0(1)*sy(-1)*sz(2)*exg(j0+1,k-1,l+2)
      ex(nn) = ex(nn) + sx0(-1)*sy(0)*sz(2)*exg(j0-1,k,l+2)
      ex(nn) = ex(nn) + sx0(0)*sy(0)*sz(2)*exg(j0,k,l+2)
      ex(nn) = ex(nn) + sx0(1)*sy(0)*sz(2)*exg(j0+1,k,l+2)
      ex(nn) = ex(nn) + sx0(-1)*sy(1)*sz(2)*exg(j0-1,k+1,l+2)
      ex(nn) = ex(nn) + sx0(0)*sy(1)*sz(2)*exg(j0,k+1,l+2)
      ex(nn) = ex(nn) + sx0(1)*sy(1)*sz(2)*exg(j0+1,k+1,l+2)
      ex(nn) = ex(nn) + sx0(-1)*sy(2)*sz(2)*exg(j0-1,k+2,l+2)
      ex(nn) = ex(nn) + sx0(0)*sy(2)*sz(2)*exg(j0,k+2,l+2)
      ex(nn) = ex(nn) + sx0(1)*sy(2)*sz(2)*exg(j0+1,k+2,l+2)
      ...
      ey(nn) = ey(nn) + sx(0)*sy0(-1)*sz(-1)*eyg(j,k0-1,l-1)
      ey(nn) = ey(nn) + sx(1)*sy0(-1)*sz(-1)*eyg(j+1,k0-1,l-1)
      ey(nn) = ey(nn) + sx(2)*sy0(-1)*sz(-1)*eyg(j+2,k0-1,l-1)
      ey(nn) = ey(nn) + sx(-1)*sy0(0)*sz(-1)*eyg(j-1,k0,l-1)
      ey(nn) = ey(nn) + sx(0)*sy0(0)*sz(-1)*eyg(j,k0,l-1)
      ey(nn) = ey(nn) + sx(1)*sy0(0)*sz(-1)*eyg(j+1,k0,l-1)
      ey(nn) = ey(nn) + sx(2)*sy0(0)*sz(-1)*eyg(j+2,k0,l-1)
      ey(nn) = ey(nn) + sx(-1)*sy0(1)*sz(-1)*eyg(j-1,k0+1,l-1)
      ey(nn) = ey(nn) + sx(0)*sy0(1)*sz(-1)*eyg(j,k0+1,l-1)
      ey(nn) = ey(nn) + sx(1)*sy0(1)*sz(-1)*eyg(j+1,k0+1,l-1)
      ey(nn) = ey(nn) + sx(2)*sy0(1)*sz(-1)*eyg(j+2,k0+1,l-1)
      ...
      ey(nn) = ey(nn) + sx(0)*sy0(-1)*sz(0)*eyg(j,k0-1,l)
      ey(nn) = ey(nn) + sx(1)*sy0(-1)*sz(0)*eyg(j+1,k0-1,l)
      ey(nn) = ey(nn) + sx(2)*sy0(-1)*sz(0)*eyg(j+2,k0-1,l)
      ey(nn) = ey(nn) + sx(-1)*sy0(0)*sz(0)*eyg(j-1,k0,l)
      ey(nn) = ey(nn) + sx(0)*sy0(0)*sz(0)*eyg(j,k0,l)
      ey(nn) = ey(nn) + sx(1)*sy0(0)*sz(0)*eyg(j+1,k0,l)
      ey(nn) = ey(nn) + sx(2)*sy0(0)*sz(0)*eyg(j+2,k0,l)
      ey(nn) = ey(nn) + sx(-1)*sy0(1)*sz(0)*eyg(j-1,k0+1,l)
      ey(nn) = ey(nn) + sx(0)*sy0(1)*sz(0)*eyg(j,k0+1,l)
      ey(nn) = ey(nn) + sx(1)*sy0(1)*sz(0)*eyg(j+1,k0+1,l)
      ey(nn) = ey(nn) + sx(2)*sy0(1)*sz(0)*eyg(j+2,k0+1,l)
      ...
      ey(nn) = ey(nn) + sx(1)*sy0(1)*sz(1)*eyg(j+1,k0+1,l+1)
      ey(nn) = ey(nn) + sx(2)*sy0(1)*sz(1)*eyg(j+2,k0+1,l+1)
      ey(nn) = ey(nn) + sx(-1)*sy0(-1)*sz(2)*eyg(j-1,k0-1,l+2)
      ey(nn) = ey(nn) + sx(0)*sy0(-1)*sz(2)*eyg(j,k0-1,l+2)
      ey(nn) = ey(nn) + sx(1)*sy0(-1)*sz(2)*eyg(j+1,k0-1,l+2)
      ey(nn) = ey(nn) + sx(2)*sy0(-1)*sz(2)*eyg(j+2,k0-1,l+2)
      ey(nn) = ey(nn) + sx(-1)*sy0(0)*sz(2)*eyg(j-1,k0,l+2)
      ey(nn) = ey(nn) + sx(0)*sy0(0)*sz(2)*eyg(j,k0,l+2)
      ey(nn) = ey(nn) + sx(1)*sy0(0)*sz(2)*eyg(j+1,k0,l+2)
      ey(nn) = ey(nn) + sx(2)*sy0(0)*sz(2)*eyg(j+2,k0,l+2)
      ey(nn) = ey(nn) + sx(-1)*sy0(1)*sz(2)*eyg(j-1,k0+1,l+2)
      ey(nn) = ey(nn) + sx(0)*sy0(1)*sz(2)*eyg(j,k0+1,l+2)
      ey(nn) = ey(nn) + sx(1)*sy0(1)*sz(2)*eyg(j+1,k0+1,l+2)
      ey(nn) = ey(nn) + sx(2)*sy0(1)*sz(2)*eyg(j+2,k0+1,l+2)
      ...
      ez(nn) = ez(nn) + sx(0)*sy(-1)*sz0(-1)*ezg(j,k-1,l0-1)
      ez(nn) = ez(nn) + sx(1)*sy(-1)*sz0(-1)*ezg(j+1,k-1,l0-1)
      ez(nn) = ez(nn) + sx(2)*sy(-1)*sz0(-1)*ezg(j+2,k-1,l0-1)
      ez(nn) = ez(nn) + sx(-1)*sy(0)*sz0(-1)*ezg(j-1,k,l0-1)
      ez(nn) = ez(nn) + sx(0)*sy(0)*sz0(-1)*ezg(j,k,l0-1)
      ez(nn) = ez(nn) + sx(1)*sy(0)*sz0(-1)*ezg(j+1,k,l0-1)
      ez(nn) = ez(nn) + sx(2)*sy(0)*sz0(-1)*ezg(j+2,k,l0-1)
      ez(nn) = ez(nn) + sx(-1)*sy(1)*sz0(-1)*ezg(j-1,k+1,l0-1)
      ez(nn) = ez(nn) + sx(0)*sy(1)*sz0(-1)*ezg(j,k+1,l0-1)
      ez(nn) = ez(nn) + sx(1)*sy(1)*sz0(-1)*ezg(j+1,k+1,l0-1)
      ez(nn) = ez(nn) + sx(2)*sy(1)*sz0(-1)*ezg(j+2,k+1,l0-1)
      ...
      ez(nn) = ez(nn) + sx(0)*sy(-1)*sz0(0)*ezg(j,k-1,l0)
      ez(nn) = ez(nn) + sx(1)*sy(-1)*sz0(0)*ezg(j+1,k-1,l0)
      ez(nn) = ez(nn) + sx(2)*sy(-1)*sz0(0)*ezg(j+2,k-1,l0)
      ez(nn) = ez(nn) + sx(-1)*sy(0)*sz0(0)*ezg(j-1,k,l0)
      ez(nn) = ez(nn) + sx(0)*sy(0)*sz0(0)*ezg(j,k,l0)
      ez(nn) = ez(nn) + sx(1)*sy(0)*sz0(0)*ezg(j+1,k,l0)
      ez(nn) = ez(nn) + sx(2)*sy(0)*sz0(0)*ezg(j+2,k,l0)
      ez(nn) = ez(nn) + sx(-1)*sy(1)*sz0(0)*ezg(j-1,k+1,l0)
      ez(nn) = ez(nn) + sx(0)*sy(1)*sz0(0)*ezg(j,k+1,l0)
      ez(nn) = ez(nn) + sx(1)*sy(1)*sz0(0)*ezg(j+1,k+1,l0)
      ez(nn) = ez(nn) + sx(2)*sy(1)*sz0(0)*ezg(j+2,k+1,l0)
      ez(nn) = ez(nn) + sx(-1)*sy(2)*sz0(0)*ezg(j-1,k+2,l0)
      ez(nn) = ez(nn) + sx(0)*sy(2)*sz0(0)*ezg(j,k+2,l0)
      ez(nn) = ez(nn) + sx(1)*sy(2)*sz0(0)*ezg(j+1,k+2,l0)
      ez(nn) = ez(nn) + sx(2)*sy(2)*sz0(0)*ezg(j+2,k+2,l0)
      ez(nn) = ez(nn) + sx(-1)*sy(-1)*sz0(1)*ezg(j-1,k-1,l0+1)
      ez(nn) = ez(nn) + sx(0)*sy(-1)*sz0(1)*ezg(j,k-1,l0+1)
      ez(nn) = ez(nn) + sx(1)*sy(-1)*sz0(1)*ezg(j+1,k-1,l0+1)
      ez(nn) = ez(nn) + sx(2)*sy(-1)*sz0(1)*ezg(j+2,k-1,l0+1)
      ...
      bx(nn) = bx(nn) + sx(0)*sy0(-1)*sz0(-1)*bxg(j,k0-1,l0-1)
      bx(nn) = bx(nn) + sx(1)*sy0(-1)*sz0(-1)*bxg(j+1,k0-1,l0-1)
      bx(nn) = bx(nn) + sx(2)*sy0(-1)*sz0(-1)*bxg(j+2,k0-1,l0-1)
      bx(nn) = bx(nn) + sx(-1)*sy0(0)*sz0(-1)*bxg(j-1,k0,l0-1)
      bx(nn) = bx(nn) + sx(0)*sy0(0)*sz0(-1)*bxg(j,k0,l0-1)
      bx(nn) = bx(nn) + sx(1)*sy0(0)*sz0(-1)*bxg(j+1,k0,l0-1)
      bx(nn) = bx(nn) + sx(2)*sy0(0)*sz0(-1)*bxg(j+2,k0,l0-1)
      bx(nn) = bx(nn) + sx(-1)*sy0(1)*sz0(-1)*bxg(j-1,k0+1,l0-1)
      bx(nn) = bx(nn) + sx(0)*sy0(1)*sz0(-1)*bxg(j,k0+1,l0-1)
      bx(nn) = bx(nn) + sx(1)*sy0(1)*sz0(-1)*bxg(j+1,k0+1,l0-1)
      bx(nn) = bx(nn) + sx(2)*sy0(1)*sz0(-1)*bxg(j+2,k0+1,l0-1)
      ...
      bx(nn) = bx(nn) + sx(0)*sy0(-1)*sz0(0)*bxg(j,k0-1,l0)
      bx(nn) = bx(nn) + sx(1)*sy0(-1)*sz0(0)*bxg(j+1,k0-1,l0)
      bx(nn) = bx(nn) + sx(2)*sy0(-1)*sz0(0)*bxg(j+2,k0-1,l0)
      bx(nn) = bx(nn) + sx(-1)*sy0(0)*sz0(0)*bxg(j-1,k0,l0)
      bx(nn) = bx(nn) + sx(0)*sy0(0)*sz0(0)*bxg(j,k0,l0)
      bx(nn) = bx(nn) + sx(1)*sy0(0)*sz0(0)*bxg(j+1,k0,l0)
      bx(nn) = bx(nn) + sx(2)*sy0(0)*sz0(0)*bxg(j+2,k0,l0)
      bx(nn) = bx(nn) + sx(-1)*sy0(1)*sz0(0)*bxg(j-1,k0+1,l0)
      bx(nn) = bx(nn) + sx(0)*sy0(1)*sz0(0)*bxg(j,k0+1,l0)
      bx(nn) = bx(nn) + sx(1)*sy0(1)*sz0(0)*bxg(j+1,k0+1,l0)
      bx(nn) = bx(nn) + sx(2)*sy0(1)*sz0(0)*bxg(j+2,k0+1,l0)
      ...
      by(nn) = by(nn) + sx0(0)*sy(1)*sz0(-1)*byg(j0,k+1,l0-1)
      by(nn) = by(nn) + sx0(1)*sy(1)*sz0(-1)*byg(j0+1,k+1,l0-1)
      by(nn) = by(nn) + sx0(-1)*sy(2)*sz0(-1)*byg(j0-1,k+2,l0-1)
      by(nn) = by(nn) + sx0(0)*sy(2)*sz0(-1)*byg(j0,k+2,l0-1)
      by(nn) = by(nn) + sx0(1)*sy(2)*sz0(-1)*byg(j0+1,k+2,l0-1)
      by(nn) = by(nn) + sx0(-1)*sy(-1)*sz0(0)*byg(j0-1,k-1,l0)
      by(nn) = by(nn) + sx0(0)*sy(-1)*sz0(0)*byg(j0,k-1,l0)
      by(nn) = by(nn) + sx0(1)*sy(-1)*sz0(0)*byg(j0+1,k-1,l0)
      ...
      by(nn) = by(nn) + sx0(0)*sy(1)*sz0(0)*byg(j0,k+1,l0)
      by(nn) = by(nn) + sx0(1)*sy(1)*sz0(0)*byg(j0+1,k+1,l0)
      by(nn) = by(nn) + sx0(-1)*sy(2)*sz0(0)*byg(j0-1,k+2,l0)
      by(nn) = by(nn) + sx0(0)*sy(2)*sz0(0)*byg(j0,k+2,l0)
      by(nn) = by(nn) + sx0(1)*sy(2)*sz0(0)*byg(j0+1,k+2,l0)
      by(nn) = by(nn) + sx0(-1)*sy(-1)*sz0(1)*byg(j0-1,k-1,l0+1)
      by(nn) = by(nn) + sx0(0)*sy(-1)*sz0(1)*byg(j0,k-1,l0+1)
      by(nn) = by(nn) + sx0(1)*sy(-1)*sz0(1)*byg(j0+1,k-1,l0+1)
      ...
      bz(nn) = bz(nn) + sx0(0)*sy0(1)*sz(1)*bzg(j0,k0+1,l+1)
      bz(nn) = bz(nn) + sx0(1)*sy0(1)*sz(1)*bzg(j0+1,k0+1,l+1)
      bz(nn) = bz(nn) + sx0(-1)*sy0(-1)*sz(2)*bzg(j0-1,k0-1,l+2)
      bz(nn) = bz(nn) + sx0(0)*sy0(-1)*sz(2)*bzg(j0,k0-1,l+2)
      bz(nn) = bz(nn) + sx0(1)*sy0(-1)*sz(2)*bzg(j0+1,k0-1,l+2)
      bz(nn) = bz(nn) + sx0(-1)*sy0(0)*sz(2)*bzg(j0-1,k0,l+2)
      bz(nn) = bz(nn) + sx0(0)*sy0(0)*sz(2)*bzg(j0,k0,l+2)
      bz(nn) = bz(nn) + sx0(1)*sy0(0)*sz(2)*bzg(j0+1,k0,l+2)
      bz(nn) = bz(nn) + sx0(-1)*sy0(1)*sz(2)*bzg(j0-1,k0+1,l+2)
      bz(nn) = bz(nn) + sx0(0)*sy0(1)*sz(2)*bzg(j0,k0+1,l+2)
      bz(nn) = bz(nn) + sx0(1)*sy0(1)*sz(2)*bzg(j0+1,k0+1,l+2)
...
!DIR$ DISTRIBUTE POINT
    ...
    ex(nn) = ex(nn) + sx0(0)*sy(-1)*sz(-1)*exg(j0,k-1,l-1)
    ex(nn) = ex(nn) + sx0(1)*sy(-1)*sz(-1)*exg(j0+1,k-1,l-1)
    ex(nn) = ex(nn) + sx0(2)*sy(-1)*sz(-1)*exg(j0+2,k-1,l-1)
    ex(nn) = ex(nn) + sx0(-1)*sy(0)*sz(-1)*exg(j0-1,k,l-1)
    ex(nn) = ex(nn) + sx0(0)*sy(0)*sz(-1)*exg(j0,k,l-1)
    ex(nn) = ex(nn) + sx0(1)*sy(0)*sz(-1)*exg(j0+1,k,l-1)
    ex(nn) = ex(nn) + sx0(2)*sy(0)*sz(-1)*exg(j0+2,k,l-1)
    ex(nn) = ex(nn) + sx0(-1)*sy(1)*sz(-1)*exg(j0-1,k+1,l-1)
    ex(nn) = ex(nn) + sx0(0)*sy(1)*sz(-1)*exg(j0,k+1,l-1)
    ex(nn) = ex(nn) + sx0(1)*sy(1)*sz(-1)*exg(j0+1,k+1,l-1)
    ex(nn) = ex(nn) + sx0(2)*sy(1)*sz(-1)*exg(j0+2,k+1,l-1)
    ...
    ex(nn) = ex(nn) + sx0(0)*sy(2)*sz(-1)*exg(j0,k+2,l-1)
    ex(nn) = ex(nn) + sx0(1)*sy(2)*sz(-1)*exg(j0+1,k+2,l-1)
    ex(nn) = ex(nn) + sx0(2)*sy(2)*sz(-1)*exg(j0+2,k+2,l-1)
    ex(nn) = ex(nn) + sx0(-1)*sy(-1)*sz(0)*exg(j0-1,k-1,l)
    ex(nn) = ex(nn) + sx0(0)*sy(-1)*sz(0)*exg(j0,k-1,l)
    ex(nn) = ex(nn) + sx0(1)*sy(-1)*sz(0)*exg(j0+1,k-1,l)
    ex(nn) = ex(nn) + sx0(2)*sy(-1)*sz(0)*exg(j0+2,k-1,l)
    ex(nn) = ex(nn) + sx0(-1)*sy(0)*sz(0)*exg(j0-1,k,l)
    ex(nn) = ex(nn) + sx0(0)*sy(0)*sz(0)*exg(j0,k,l)
    ex(nn) = ex(nn) + sx0(1)*sy(0)*sz(0)*exg(j0+1,k,l)
    ex(nn) = ex(nn) + sx0(2)*sy(0)*sz(0)*exg(j0+2,k,l)
    ex(nn) = ex(nn) + sx0(-1)*sy(1)*sz(0)*exg(j0-1,k+1,l)
    ex(nn) = ex(nn) + sx0(0)*sy(1)*sz(0)*exg(j0,k+1,l)
    ex(nn) = ex(nn) + sx0(1)*sy(1)*sz(0)*exg(j0+1,k+1,l)
    ex(nn) = ex(nn) + sx0(2)*sy(1)*sz(0)*exg(j0+2,k+1,l)
    ...
    ex(nn) = ex(nn) + sx0(0)*sy(2)*sz(0)*exg(j0,k+2,l)
    ex(nn) = ex(nn) + sx0(1)*sy(2)*sz(0)*exg(j0+1,k+2,l)
    ex(nn) = ex(nn) + sx0(2)*sy(2)*sz(0)*exg(j0+2,k+2,l)
    ex(nn) = ex(nn) + sx0(-1)*sy(-1)*sz(1)*exg(j0-1,k-1,l+1)
    ex(nn) = ex(nn) + sx0(0)*sy(-1)*sz(1)*exg(j0,k-1,l+1)
    ex(nn) = ex(nn) + sx0(1)*sy(-1)*sz(1)*exg(j0+1,k-1,l+1)
    ex(nn) = ex(nn) + sx0(2)*sy(-1)*sz(1)*exg(j0+2,k-1,l+1)
    ...
    ex(nn) = ex(nn) + sx0(0)*sy(-1)*sz(2)*exg(j0,k-1,l+2)
    ex(nn) = ex(nn) + sx0(1)*sy(-1)*sz(2)*exg(j0+1,k-1,l+2)
    ex(nn) = ex(nn) + sx0(2)*sy(-1)*sz(2)*exg(j0+2,k-1,l+2)
    ex(nn) = ex(nn) + sx0(-1)*sy(0)*sz(2)*exg(j0-1,k,l+2)
    ex(nn) = ex(nn) + sx0(0)*sy(0)*sz(2)*exg(j0,k,l+2)
    ex(nn) = ex(nn) + sx0(1)*sy(0)*sz(2)*exg(j0+1,k,l+2)
    ex(nn) = ex(nn) + sx0(2)*sy(0)*sz(2)*exg(j0+2,k,l+2)
    ex(nn) = ex(nn) + sx0(-1)*sy(1)*sz(2)*exg(j0-1,k+1,l+2)
    ex(nn) = ex(nn) + sx0(0)*sy(1)*sz(2)*exg(j0,k+1,l+2)
    ex(nn) = ex(nn) + sx0(1)*sy(1)*sz(2)*exg(j0+1,k+1,l+2)
    ex(nn) = ex(nn) + sx0(2)*sy(1)*sz(2)*exg(j0+2,k+1,l+2)
    ...
    ex(nn) = ex(nn) + sx0(0)*sy(2)*sz(2)*exg(j0,k+2,l+2)
    ex(nn) = ex(nn) + sx0(1)*sy(2)*sz(2)*exg(j0+1,k+2,l+2)
    ex(nn) = ex(nn) + sx0(2)*sy(2)*sz(2)*exg(j0+2,k+2,l+2)
    ...
    ey(nn) = ey(nn) + sx(1)*sy0(1)*sz(-1)*eyg(j+1,k0+1,l-1)
    ey(nn) = ey(nn) + sx(2)*sy0(1)*sz(-1)*eyg(j+2,k0+1,l-1)
    ey(nn) = ey(nn) + sx(-1)*sy0(2)*sz(-1)*eyg(j-1,k0+2,l-1)
    ey(nn) = ey(nn) + sx(0)*sy0(2)*sz(-1)*eyg(j,k0+2,l-1)
    ey(nn) = ey(nn) + sx(1)*sy0(2)*sz(-1)*eyg(j+1,k0+2,l-1)
    ey(nn) = ey(nn) + sx(2)*sy0(2)*sz(-1)*eyg(j+2,k0+2,l-1)
    ey(nn) = ey(nn) + sx(-1)*sy0(-1)*sz(0)*eyg(j-1,k0-1,l)
    ey(nn) = ey(nn) + sx(0)*sy0(-1)*sz(0)*eyg(j,k0-1,l)
    ey(nn) = ey(nn) + sx(1)*sy0(-1)*sz(0)*eyg(j+1,k0-1,l)
    ey(nn) = ey(nn) + sx(2)*sy0(-1)*sz(0)*eyg(j+2,k0-1,l)
    ...
    ey(nn) = ey(nn) + sx(1)*sy0(1)*sz(0)*eyg(j+1,k0+1,l)
    ey(nn) = ey(nn) + sx(2)*sy0(1)*sz(0)*eyg(j+2,k0+1,l)
    ey(nn) = ey(nn) + sx(-1)*sy0(2)*sz(0)*eyg(j-1,k0+2,l)
    ey(nn) = ey(nn) + sx(0)*sy0(2)*sz(0)*eyg(j,k0+2,l)
    ey(nn) = ey(nn) + sx(1)*sy0(2)*sz(0)*eyg(j+1,k0+2,l)
    ey(nn) = ey(nn) + sx(2)*sy0(2)*sz(0)*eyg(j+2,k0+2,l)
    ey(nn) = ey(nn) + sx(-1)*sy0(-1)*sz(1)*eyg(j-1,k0-1,l+1)
    ey(nn) = ey(nn) + sx(0)*sy0(-1)*sz(1)*eyg(j,k0-1,l+1)
    ey(nn) = ey(nn) + sx(1)*sy0(-1)*sz(1)*eyg(j+1,k0-1,l+1)
    ey(nn) = ey(nn) + sx(2)*sy0(-1)*sz(1)*eyg(j+2,k0-1,l+1)
    ...
    ey(nn) = ey(nn) + sx(1)*sy0(1)*sz(2)*eyg(j+1,k0+1,l+2)
    ey(nn) = ey(nn) + sx(2)*sy0(1)*sz(2)*eyg(j+2,k0+1,l+2)
    ey(nn) = ey(nn) + sx(-1)*sy0(2)*sz(2)*eyg(j-1,k0+2,l+2)
    ey(nn) = ey(nn) + sx(0)*sy0(2)*sz(2)*eyg(j,k0+2,l+2)
    ey(nn) = ey(nn) + sx(1)*sy0(2)*sz(2)*eyg(j+1,k0+2,l+2)
    ey(nn) = ey(nn) + sx(2)*sy0(2)*sz(2)*eyg(j+2,k0+2,l+2)
    ...
    ez(nn) = ez(nn) + sx(1)*sy(2)*sz0(1)*ezg(j+1,k+2,l0+1)
    ez(nn) = ez(nn) + sx(2)*sy(2)*sz0(1)*ezg(j+2,k+2,l0+1)
    ez(nn) = ez(nn) + sx(-1)*sy(-1)*sz0(2)*ezg(j-1,k-1,l0+2)
    ez(nn) = ez(nn) + sx(0)*sy(-1)*sz0(2)*ezg(j,k-1,l0+2)
    ez(nn) = ez(nn) + sx(1)*sy(-1)*sz0(2)*ezg(j+1,k-1,l0+2)
    ez(nn) = ez(nn) + sx(2)*sy(-1)*sz0(2)*ezg(j+2,k-1,l0+2)
    ez(nn) = ez(nn) + sx(-1)*sy(0)*sz0(2)*ezg(j-1,k,l0+2)
    ez(nn) = ez(nn) + sx(0)*sy(0)*sz0(2)*ezg(j,k,l0+2)
    ez(nn) = ez(nn) + sx(1)*sy(0)*sz0(2)*ezg(j+1,k,l0+2)
    ez(nn) = ez(nn) + sx(2)*sy(0)*sz0(2)*ezg(j+2,k,l0+2)
    ez(nn) = ez(nn) + sx(-1)*sy(1)*sz0(2)*ezg(j-1,k+1,l0+2)
    ez(nn) = ez(nn) + sx(0)*sy(1)*sz0(2)*ezg(j,k+1,l0+2)
    ez(nn) = ez(nn) + sx(1)*sy(1)*sz0(2)*ezg(j+1,k+1,l0+2)
    ez(nn) = ez(nn) + sx(2)*sy(1)*sz0(2)*ezg(j+2,k+1,l0+2)
    ez(nn) = ez(nn) + sx(-1)*sy(2)*sz0(2)*ezg(j-1,k+2,l0+2)
    ez(nn) = ez(nn) + sx(0)*sy(2)*sz0(2)*ezg(j,k+2,l0+2)
    ez(nn) = ez(nn) + sx(1)*sy(2)*sz0(2)*ezg(j+1,k+2,l0+2)
    ez(nn) = ez(nn) + sx(2)*sy(2)*sz0(2)*ezg(j+2,k+2,l0+2)
    ...
    bx(nn) = bx(nn) + sx(1)*sy0(1)*sz0(-1)*bxg(j+1,k0+1,l0-1)
    bx(nn) = bx(nn) + sx(2)*sy0(1)*sz0(-1)*bxg(j+2,k0+1,l0-1)
    bx(nn) = bx(nn) + sx(-1)*sy0(2)*sz0(-1)*bxg(j-1,k0+2,l0-1)
    bx(nn) = bx(nn) + sx(0)*sy0(2)*sz0(-1)*bxg(j,k0+2,l0-1)
    bx(nn) = bx(nn) + sx(1)*sy0(2)*sz0(-1)*bxg(j+1,k0+2,l0-1)
    bx(nn) = bx(nn) + sx(2)*sy0(2)*sz0(-1)*bxg(j+2,k0+2,l0-1)
    bx(nn) = bx(nn) + sx(-1)*sy0(-1)*sz0(0)*bxg(j-1,k0-1,l0)
    bx(nn) = bx(nn) + sx(0)*sy0(-1)*sz0(0)*bxg(j,k0-1,l0)
    bx(nn) = bx(nn) + sx(1)*sy0(-1)*sz0(0)*bxg(j+1,k0-1,l0)
    bx(nn) = bx(nn) + sx(2)*sy0(-1)*sz0(0)*bxg(j+2,k0-1,l0)
    ...
    bx(nn) = bx(nn) + sx(1)*sy0(1)*sz0(0)*bxg(j+1,k0+1,l0)
    bx(nn) = bx(nn) + sx(2)*sy0(1)*sz0(0)*bxg(j+2,k0+1,l0)
    bx(nn) = bx(nn) + sx(-1)*sy0(2)*sz0(0)*bxg(j-1,k0+2,l0)
    bx(nn) = bx(nn) + sx(0)*sy0(2)*sz0(0)*bxg(j,k0+2,l0)
    bx(nn) = bx(nn) + sx(1)*sy0(2)*sz0(0)*bxg(j+1,k0+2,l0)
    bx(nn) = bx(nn) + sx(2)*sy0(2)*sz0(0)*bxg(j+2,k0+2,l0)
    bx(nn) = bx(nn) + sx(-1)*sy0(-1)*sz0(1)*bxg(j-1,k0-1,l0+1)
    bx(nn) = bx(nn) + sx(0)*sy0(-1)*sz0(1)*bxg(j,k0-1,l0+1)
    bx(nn) = bx(nn) + sx(1)*sy0(-1)*sz0(1)*bxg(j+1,k0-1,l0+1)
    bx(nn) = bx(nn) + sx(2)*sy0(-1)*sz0(1)*bxg(j+2,k0-1,l0+1)
    ...
    bx(nn) = bx(nn) + sx(1)*sy0(2)*sz0(1)*bxg(j+1,k0+2,l0+1)
    bx(nn) = bx(nn) + sx(2)*sy0(2)*sz0(1)*bxg(j+2,k0+2,l0+1)
    bx(nn) = bx(nn) + sx(-1)*sy0(-1)*sz0(2)*bxg(j-1,k0-1,l0+2)
    bx(nn) = bx(nn) + sx(0)*sy0(-1)*sz0(2)*bxg(j,k0-1,l0+2)
    bx(nn) = bx(nn) + sx(1)*sy0(-1)*sz0(2)*bxg(j+1,k0-1,l0+2)
    bx(nn) = bx(nn) + sx(2)*sy0(-1)*sz0(2)*bxg(j+2,k0-1,l0+2)
    bx(nn) = bx(nn) + sx(-1)*sy0(0)*sz0(2)*bxg(j-1,k0,l0+2)
    bx(nn) = bx(nn) + sx(0)*sy0(0)*sz0(2)*bxg(j,k0,l0+2)
    bx(nn) = bx(nn) + sx(1)*sy0(0)*sz0(2)*bxg(j+1,k0,l0+2)
    bx(nn) = bx(nn) + sx(2)*sy0(0)*sz0(2)*bxg(j+2,k0,l0+2)
    bx(nn) = bx(nn) + sx(-1)*sy0(1)*sz0(2)*bxg(j-1,k0+1,l0+2)
    bx(nn) = bx(nn) + sx(0)*sy0(1)*sz0(2)*bxg(j,k0+1,l0+2)
    bx(nn) = bx(nn) + sx(1)*sy0(1)*sz0(2)*bxg(j+1,k0+1,l0+2)
    bx(nn) = bx(nn) + sx(2)*sy0(1)*sz0(2)*bxg(j+2,k0+1,l0+2)
    bx(nn) = bx(nn) + sx(-1)*sy0(2)*sz0(2)*bxg(j-1,k0+2,l0+2)
    bx(nn) = bx(nn) + sx(0)*sy0(2)*sz0(2)*bxg(j,k0+2,l0+2)
    bx(nn) = bx(nn) + sx(1)*sy0(2)*sz0(2)*bxg(j+1,k0+2,l0+2)
    bx(nn) = bx(nn) + sx(2)*sy0(2)*sz0(2)*bxg(j+2,k0+2,l0+2)
    
    ! Compute By on particle
    by(nn) = by(nn) + sx0(-1)*sy(-1)*sz0(-1)*byg(j0-1,k-1,l0-1)
    by(nn) = by(nn) + sx0(0)*sy(-1)*sz0(-1)*byg(j0,k-1,l0-1)
    by(nn) = by(nn) + sx0(1)*sy(-1)*sz0(-1)*byg(j0+1,k-1,l0-1)
    by(nn) = by(nn) + sx0(2)*sy(-1)*sz0(-1)*byg(j0+2,k-1,l0-1)
    by(nn) = by(nn) + sx0(-1)*sy(0)*sz0(-1)*byg(j0-1,k,l0-1)
    by(nn) = by(nn) + sx0(0)*sy(0)*sz0(-1)*byg(j0,k,l0-1)
    by(nn) = by(nn) + sx0(1)*sy(0)*sz0(-1)*byg(j0+1,k,l0-1)
    by(nn) = by(nn) + sx0(2)*sy(0)*sz0(-1)*byg(j0+2,k,l0-1)
    by(nn) = by(nn) + sx0(-1)*sy(1)*sz0(-1)*byg(j0-1,k+1,l0-1)
    by(nn) = by(nn) + sx0(0)*sy(1)*sz0(-1)*byg(j0,k+1,l0-1)
    by(nn) = by(nn) + sx0(1)*sy(1)*sz0(-1)*byg(j0+1,k+1,l0-1)
    by(nn) = by(nn) + sx0(2)*sy(1)*sz0(-1)*byg(j0+2,k+1,l0-1)
    ...
    by(nn) = by(nn) + sx0(0)*sy(2)*sz0(-1)*byg(j0,k+2,l0-1)
    by(nn) = by(nn) + sx0(1)*sy(2)*sz0(-1)*byg(j0+1,k+2,l0-1)
    by(nn) = by(nn) + sx0(2)*sy(2)*sz0(-1)*byg(j0+2,k+2,l0-1)
    by(nn) = by(nn) + sx0(-1)*sy(-1)*sz0(0)*byg(j0-1,k-1,l0)
    by(nn) = by(nn) + sx0(0)*sy(-1)*sz0(0)*byg(j0,k-1,l0)
    by(nn) = by(nn) + sx0(1)*sy(-1)*sz0(0)*byg(j0+1,k-1,l0)
    by(nn) = by(nn) + sx0(2)*sy(-1)*sz0(0)*byg(j0+2,k-1,l0)
    by(nn) = by(nn) + sx0(-1)*sy(0)*sz0(0)*byg(j0-1,k,l0)
    by(nn) = by(nn) + sx0(0)*sy(0)*sz0(0)*byg(j0,k,l0)
    by(nn) = by(nn) + sx0(1)*sy(0)*sz0(0)*byg(j0+1,k,l0)
    by(nn) = by(nn) + sx0(2)*sy(0)*sz0(0)*byg(j0+2,k,l0)
    by(nn) = by(nn) + sx0(-1)*sy(1)*sz0(0)*byg(j0-1,k+1,l0)
    by(nn) = by(nn) + sx0(0)*sy(1)*sz0(0)*byg(j0,k+1,l0)
    by(nn) = by(nn) + sx0(1)*sy(1)*sz0(0)*byg(j0+1,k+1,l0)
    by(nn) = by(nn) + sx0(2)*sy(1)*sz0(0)*byg(j0+2,k+1,l0)
    ...
    by(nn) = by(nn) + sx0(0)*sy(2)*sz0(0)*byg(j0,k+2,l0)
    by(nn) = by(nn) + sx0(1)*sy(2)*sz0(0)*byg(j0+1,k+2,l0)
    by(nn) = by(nn) + sx0(2)*sy(2)*sz0(0)*byg(j0+2,k+2,l0)
    by(nn) = by(nn) + sx0(-1)*sy(-1)*sz0(1)*byg(j0-1,k-1,l0+1)
    by(nn) = by(nn) + sx0(0)*sy(-1)*sz0(1)*byg(j0,k-1,l0+1)
    by(nn) = by(nn) + sx0(1)*sy(-1)*sz0(1)*byg(j0+1,k-1,l0+1)
    by(nn) = by(nn) + sx0(2)*sy(-1)*sz0(1)*byg(j0+2,k-1,l0+1)
    ...
    by(nn) = by(nn) + sx0(1)*sy(2)*sz0(1)*byg(j0+1,k+2,l0+1)
    by(nn) = by(nn) + sx0(2)*sy(2)*sz0(1)*byg(j0+2,k+2,l0+1)
    by(nn) = by(nn) + sx0(-1)*sy(-1)*sz0(2)*byg(j0-1,k-1,l0+2)
    by(nn) = by(nn) + sx0(0)*sy(-1)*sz0(2)*byg(j0,k-1,l0+2)
    by(nn) = by(nn) + sx0(1)*sy(-1)*sz0(2)*byg(j0+1,k-1,l0+2)
    by(nn) = by(nn) + sx0(2)*sy(-1)*sz0(2)*byg(j0+2,k-1,l0+2)
    by(nn) = by(nn) + sx0(-1)*sy(0)*sz0(2)*byg(j0-1,k,l0+2)
    by(nn) = by(nn) + sx0(0)*sy(0)*sz0(2)*byg(j0,k,l0+2)
    by(nn) = by(nn) + sx0(1)*sy(0)*sz0(2)*byg(j0+1,k,l0+2)
    by(nn) = by(nn) + sx0(2)*sy(0)*sz0(2)*byg(j0+2,k,l0+2)
    by(nn) = by(nn) + sx0(-1)*sy(1)*sz0(2)*byg(j0-1,k+1,l0+2)
    by(nn) = by(nn) + sx0(0)*sy(1)*sz0(2)*byg(j0,k+1,l0+2)
    by(nn) = by(nn) + sx0(1)*sy(1)*sz0(2)*byg(j0+1,k+1,l0+2)
    by(nn) = by(nn) + sx0(2)*sy(1)*sz0(2)*byg(j0+2,k+1,l0+2)
    by(nn) = by(nn) + sx0(-1)*sy(2)*sz0(2)*byg(j0-1,k+2,l0+2)
    by(nn) = by(nn) + sx0(0)*sy(2)*sz0(2)*byg(j0,k+2,l0+2)
    by(nn) = by(nn) + sx0(1)*sy(2)*sz0(2)*byg(j0+1,k+2,l0+2)
    by(nn) = by(nn) + sx0(2)*sy(2)*sz0(2)*byg(j0+2,k+2,l0+2)
    
    ! Compute Bz on particle
    bz(nn) = bz(nn) + sx0(-1)*sy0(-1)*sz(-1)*bzg(j0-1,k0-1,l-1)
    bz(nn) = bz(nn) + sx0(0)*sy0(-1)*sz(-1)*bzg(j0,k0-1,l-1)
    bz(nn) = bz(nn) + sx0(1)*sy0(-1)*sz(-1)*bzg(j0+1,k0-1,l-1)
    bz(nn) = bz(nn) + sx0(2)*sy0(-1)*sz(-1)*bzg(j0+2,k0-1,l-1)
    bz(nn) = bz(nn) + sx0(-1)*sy0(0)*sz(-1)*bzg(j0-1,k0,l-1)
    bz(nn) = bz(nn) + sx0(0)*sy0(0)*sz(-1)*bzg(j0,k0,l-1)
    bz(nn) = bz(nn) + sx0(1)*sy0(0)*sz(-1)*bzg(j0+1,k0,l-1)
    bz(nn) = bz(nn) + sx0(2)*sy0(0)*sz(-1)*bzg(j0+2,k0,l-1)
    bz(nn) = bz(nn) + sx0(-1)*sy0(1)*sz(-1)*bzg(j0-1,k0+1,l-1)
    bz(nn) = bz(nn) + sx0(0)*sy0(1)*sz(-1)*bzg(j0,k0+1,l-1)
    bz(nn) = bz(nn) + sx0(1)*sy0(1)*sz(-1)*bzg(j0+1,k0+1,l-1)
    bz(nn) = bz(nn) + sx0(2)*sy0(1)*sz(-1)*bzg(j0+2,k0+1,l-1)
    bz(nn) = bz(nn) + sx0(-1)*sy0(2)*sz(-1)*bzg(j0-1,k0+2,l-1)
    bz(nn) = bz(nn) + sx0(0)*sy0(2)*sz(-1)*bzg(j0,k0+2,l-1)
    bz(nn) = bz(nn) + sx0(1)*sy0(2)*sz(-1)*bzg(j0+1,k0+2,l-1)
    bz(nn) = bz(nn) + sx0(2)*sy0(2)*sz(-1)*bzg(j0+2,k0+2,l-1)
    bz(nn) = bz(nn) + sx0(-1)*sy0(-1)*sz(0)*bzg(j0-1,k0-1,l)
    bz(nn) = bz(nn) + sx0(0)*sy0(-1)*sz(0)*bzg(j0,k0-1,l)
    bz(nn) = bz(nn) + sx0(1)*sy0(-1)*sz(0)*bzg(j0+1,k0-1,l)
    bz(nn) = bz(nn) + sx0(2)*sy0(-1)*sz(0)*bzg(j0+2,k0-1,l)
    bz(nn) = bz(nn) + sx0(-1)*sy0(0)*sz(0)*bzg(j0-1,k0,l)
    bz(nn) = bz(nn) + sx0(0)*sy0(0)*sz(0)*bzg(j0,k0,l)
    bz(nn) = bz(nn) + sx0(1)*sy0(0)*sz(0)*bzg(j0+1,k0,l)
    bz(nn) = bz(nn) + sx0(2)*sy0(0)*sz(0)*bzg(j0+2,k0,l)
    bz(nn) = bz(nn) + sx0(-1)*sy0(1)*sz(0)*bzg(j0-1,k0+1,l)
    bz(nn) = bz(nn) + sx0(0)*sy0(1)*sz(0)*bzg(j0,k0+1,l)
    bz(nn) = bz(nn) + sx0(1)*sy0(1)*sz(0)*bzg(j0+1,k0+1,l)
    bz(nn) = bz(nn) + sx0(2)*sy0(1)*sz(0)*bzg(j0+2,k0+1,l)
    bz(nn) = bz(nn) + sx0(-1)*sy0(2)*sz(0)*bzg(j0-1,k0+2,l)
    bz(nn) = bz(nn) + sx0(0)*sy0(2)*sz(0)*bzg(j0,k0+2,l)
    bz(nn) = bz(nn) + sx0(1)*sy0(2)*sz(0)*bzg(j0+1,k0+2,l)
    bz(nn) = bz(nn) + sx0(2)*sy0(2)*sz(0)*bzg(j0+2,k0+2,l)
    bz(nn) = bz(nn) + sx0(-1)*sy0(-1)*sz(1)*bzg(j0-1,k0-1,l+1)
    bz(nn) = bz(nn) + sx0(0)*sy0(-1)*sz(1)*bzg(j0,k0-1,l+1)
    bz(nn) = bz(nn) + sx0(1)*sy0(-1)*sz(1)*bzg(j0+1,k0-1,l+1)
    bz(nn) = bz(nn) + sx0(2)*sy0(-1)*sz(1)*bzg(j0+2,k0-1,l+1)
    ...
    bz(nn) = bz(nn) + sx0(0)*sy0(-1)*sz(2)*bzg(j0,k0-1,l+2)
    bz(nn) = bz(nn) + sx0(1)*sy0(-1)*sz(2)*bzg(j0+1,k0-1,l+2)
    bz(nn) = bz(nn) + sx0(2)*sy0(-1)*sz(2)*bzg(j0+2,k0-1,l+2)
    bz(nn) = bz(nn) + sx0(-1)*sy0(0)*sz(2)*bzg(j0-1,k0,l+2)
    bz(nn) = bz(nn) + sx0(0)*sy0(0)*sz(2)*bzg(j0,k0,l+2)
    bz(nn) = bz(nn) + sx0(1)*sy0(0)*sz(2)*bzg(j0+1,k0,l+2)
    bz(nn) = bz(nn) + sx0(2)*sy0(0)*sz(2)*bzg(j0+2,k0,l+2)
    bz(nn) = bz(nn) + sx0(-1)*sy0(1)*sz(2)*bzg(j0-1,k0+1,l+2)
    bz(nn) = bz(nn) + sx0(0)*sy0(1)*sz(2)*bzg(j0,k0+1,l+2)
    bz(nn) = bz(nn) + sx0(1)*sy0(1)*sz(2)*bzg(j0+1,k0+1,l+2)
    bz(nn) = bz(nn) + sx0(2)*sy0(1)*sz(2)*bzg(j0+2,k0+1,l+2)
    bz(nn) = bz(nn) + sx0(-1)*sy0(2)*sz(2)*bzg(j0-1,k0+2,l+2)
    bz(nn) = bz(nn) + sx0(0)*sy0(2)*sz(2)*bzg(j0,k0+2,l+2)
    bz(nn) = bz(nn) + sx0(1)*sy0(2)*sz(2)*bzg(j0+1,k0+2,l+2)
    bz(nn) = bz(nn) + sx0(2)*sy0(2)*sz(2)*bzg(j0+2,k0+2,l+2)
...
!DIR$ DISTRIBUTE POINT
    ...
    DO n=1,MIN(lvect,np-ip+1)
      nn=ip+n-1    
      ! Compute Ey on particle
      ey(nn) = ey(nn) + sx(n,-1)*sx0(n,-1)*sz(n,-1)*eyg(j(n)-1,k0(n)-1,l(n)-1)
      ey(nn) = ey(nn) + sx(n,0)*sx0(n,-1)*sz(n,-1)*eyg(j(n),k0(n)-1,l(n)-1)
      ey(nn) = ey(nn) + sx(n,1)*sx0(n,-1)*sz(n,-1)*eyg(j(n)+1,k0(n)-1,l(n)-1)
      ey(nn) = ey(nn) + sx(n,2)*sx0(n,-1)*sz(n,-1)*eyg(j(n)+2,k0(n)-1,l(n)-1)
      ey(nn) = ey(nn) + sx(n,-1)*sx0(n,0)*sz(n,-1)*eyg(j(n)-1,k0(n),l(n)-1)
      ey(nn) = ey(nn) + sx(n,0)*sx0(n,0)*sz(n,-1)*eyg(j(n),k0(n),l(n)-1)
      ey(nn) = ey(nn) + sx(n,1)*sx0(n,0)*sz(n,-1)*eyg(j(n)+1,k0(n),l(n)-1)
      ey(nn) = ey(nn) + sx(n,2)*sx0(n,0)*sz(n,-1)*eyg(j(n)+2,k0(n),l(n)-1)
      ey(nn) = ey(nn) + sx(n,-1)*sx0(n,1)*sz(n,-1)*eyg(j(n)-1,k0(n)+1,l(n)-1)
      ey(nn) = ey(nn) + sx(n,0)*sx0(n,1)*sz(n,-1)*eyg(j(n),k0(n)+1,l(n)-1)
      ey(nn) = ey(nn) + sx(n,1)*sx0(n,1)*sz(n,-1)*eyg(j(n)+1,k0(n)+1,l(n)-1)
      ey(nn) = ey(nn) + sx(n,2)*sx0(n,1)*sz(n,-1)*eyg(j(n)+2,k0(n)+1,l(n)-1)
      ey(nn) = ey(nn) + sx(n,-1)*sx0(n,-1)*sz(n,0)*eyg(j(n)-1,k0(n)-1,l(n))
      ey(nn) = ey(nn) + sx(n,0)*sx0(n,-1)*sz(n,0)*eyg(j(n),k0(n)-1,l(n))
      ey(nn) = ey(nn) + sx(n,1)*sx0(n,-1)*sz(n,0)*eyg(j(n)+1,k0(n)-1,l(n))
      ey(nn) = ey(nn) + sx(n,2)*sx0(n,-1)*sz(n,0)*eyg(j(n)+2,k0(n)-1,l(n))
      ey(nn) = ey(nn) + sx(n,-1)*sx0(n,0)*sz(n,0)*eyg(j(n)-1,k0(n),l(n))
      ey(nn) = ey(nn) + sx(n,0)*sx0(n,0)*sz(n,0)*eyg(j(n),k0(n),l(n))
      ey(nn) = ey(nn) + sx(n,1)*sx0(n,0)*sz(n,0)*eyg(j(n)+1,k0(n),l(n))
      ey(nn) = ey(nn) + sx(n,2)*sx0(n,0)*sz(n,0)*eyg(j(n)+2,k0(n),l(n))
      ey(nn) = ey(nn) + sx(n,-1)*sx0(n,1)*sz(n,0)*eyg(j(n)-1,k0(n)+1,l(n))
      ey(nn) = ey(nn) + sx(n,0)*sx0(n,1)*sz(n,0)*eyg(j(n),k0(n)+1,l(n))
      ey(nn) = ey(nn) + sx(n,1)*sx0(n,1)*sz(n,0)*eyg(j(n)+1,k0(n)+1,l(n))
      ey(nn) = ey(nn) + sx(n,2)*sx0(n,1)*sz(n,0)*eyg(j(n)+2,k0(n)+1,l(n))
      ...
      ey(nn) = ey(nn) + sx(n,1)*sx0(n,1)*sz(n,1)*eyg(j(n)+1,k0(n)+1,l(n)+1)
      ey(nn) = ey(nn) + sx(n,2)*sx0(n,1)*sz(n,1)*eyg(j(n)+2,k0(n)+1,l(n)+1)
      ey(nn) = ey(nn) + sx(n,-1)*sx0(n,-1)*sz(n,2)*eyg(j(n)-1,k0(n)-1,l(n)+2)
      ey(nn) = ey(nn) + sx(n,0)*sx0(n,-1)*sz(n,2)*eyg(j(n),k0(n)-1,l(n)+2)
      ey(nn) = ey(nn) + sx(n,1)*sx0(n,-1)*sz(n,2)*eyg(j(n)+1,k0(n)-1,l(n)+2)
      ey(nn) = ey(nn) + sx(n,2)*sx0(n,-1)*sz(n,2)*eyg(j(n)+2,k0(n)-1,l(n)+2)
      ey(nn) = ey(nn) + sx(n,-1)*sx0(n,0)*sz(n,2)*eyg(j(n)-1,k0(n),l(n)+2)
      ey(nn) = ey(nn) + sx(n,0)*sx0(n,0)*sz(n,2)*eyg(j(n),k0(n),l(n)+2)
      ey(nn) = ey(nn) + sx(n,1)*sx0(n,0)*sz(n,2)*eyg(j(n)+1,k0(n),l(n)+2)
      ey(nn) = ey(nn) + sx(n,2)*sx0(n,0)*sz(n,2)*eyg(j(n)+2,k0(n),l(n)+2)
      ey(nn) = ey(nn) + sx(n,-1)*sx0(n,1)*sz(n,2)*eyg(j(n)-1,k0(n)+1,l(n)+2)
      ey(nn) = ey(nn) + sx(n,0)*sx0(n,1)*sz(n,2)*eyg(j(n),k0(n)+1,l(n)+2)
      ey(nn) = ey(nn) + sx(n,1)*sx0(n,1)*sz(n,2)*eyg(j(n)+1,k0(n)+1,l(n)+2)
      ey(nn) = ey(nn) + sx(n,2)*sx0(n,1)*sz(n,2)*eyg(j(n)+2,k0(n)+1,l(n)+2)
...
!DIR$ DISTRIBUTE POINT
    ...
    DO n=1,MIN(lvect,np-ip+1)
      ...
      ! Compute Bx on particle
      bx(nn) = bx(nn) + sx(n,-1)*sx0(n,-1)*sz0(n,-1)*bxg(j(n)-1,k0(n)-1,l0(n)-1)
      bx(nn) = bx(nn) + sx(n,0)*sx0(n,-1)*sz0(n,-1)*bxg(j(n),k0(n)-1,l0(n)-1)
      bx(nn) = bx(nn) + sx(n,1)*sx0(n,-1)*sz0(n,-1)*bxg(j(n)+1,k0(n)-1,l0(n)-1)
      bx(nn) = bx(nn) + sx(n,2)*sx0(n,-1)*sz0(n,-1)*bxg(j(n)+2,k0(n)-1,l0(n)-1)
      bx(nn) = bx(nn) + sx(n,-1)*sx0(n,0)*sz0(n,-1)*bxg(j(n)-1,k0(n),l0(n)-1)
      bx(nn) = bx(nn) + sx(n,0)*sx0(n,0)*sz0(n,-1)*bxg(j(n),k0(n),l0(n)-1)
      bx(nn) = bx(nn) + sx(n,1)*sx0(n,0)*sz0(n,-1)*bxg(j(n)+1,k0(n),l0(n)-1)
      bx(nn) = bx(nn) + sx(n,2)*sx0(n,0)*sz0(n,-1)*bxg(j(n)+2,k0(n),l0(n)-1)
      bx(nn) = bx(nn) + sx(n,-1)*sx0(n,1)*sz0(n,-1)*bxg(j(n)-1,k0(n)+1,l0(n)-1)
      bx(nn) = bx(nn) + sx(n,0)*sx0(n,1)*sz0(n,-1)*bxg(j(n),k0(n)+1,l0(n)-1)
      bx(nn) = bx(nn) + sx(n,1)*sx0(n,1)*sz0(n,-1)*bxg(j(n)+1,k0(n)+1,l0(n)-1)
      bx(nn) = bx(nn) + sx(n,2)*sx0(n,1)*sz0(n,-1)*bxg(j(n)+2,k0(n)+1,l0(n)-1)
      bx(nn) = bx(nn) + sx(n,-1)*sx0(n,-1)*sz0(n,0)*bxg(j(n)-1,k0(n)-1,l0(n))
      bx(nn) = bx(nn) + sx(n,0)*sx0(n,-1)*sz0(n,0)*bxg(j(n),k0(n)-1,l0(n))
      bx(nn) = bx(nn) + sx(n,1)*sx0(n,-1)*sz0(n,0)*bxg(j(n)+1,k0(n)-1,l0(n))
      bx(nn) = bx(nn) + sx(n,2)*sx0(n,-1)*sz0(n,0)*bxg(j(n)+2,k0(n)-1,l0(n))
      bx(nn) = bx(nn) + sx(n,-1)*sx0(n,0)*sz0(n,0)*bxg(j(n)-1,k0(n),l0(n))
      bx(nn) = bx(nn) + sx(n,0)*sx0(n,0)*sz0(n,0)*bxg(j(n),k0(n),l0(n))
      bx(nn) = bx(nn) + sx(n,1)*sx0(n,0)*sz0(n,0)*bxg(j(n)+1,k0(n),l0(n))
      bx(nn) = bx(nn) + sx(n,2)*sx0(n,0)*sz0(n,0)*bxg(j(n)+2,k0(n),l0(n))
      bx(nn) = bx(nn) + sx(n,-1)*sx0(n,1)*sz0(n,0)*bxg(j(n)-1,k0(n)+1,l0(n))
      bx(nn) = bx(nn) + sx(n,0)*sx0(n,1)*sz0(n,0)*bxg(j(n),k0(n)+1,l0(n))
      bx(nn) = bx(nn) + sx(n,1)*sx0(n,1)*sz0(n,0)*bxg(j(n)+1,k0(n)+1,l0(n))
      bx(nn) = bx(nn) + sx(n,2)*sx0(n,1)*sz0(n,0)*bxg(j(n)+2,k0(n)+1,l0(n))
...
!DIR$ DISTRIBUTE POINT
    ...
    DO n=1,MIN(lvect,np-ip+1)
      nn=ip+n-1  
      ! Compute Bz on particle
      bz(nn) = bz(nn) + sx0(n,-1)*sx0(n,-1)*sz(n,-1)*bzg(j0(n)-1,k0(n)-1,l(n)-1)
      bz(nn) = bz(nn) + sx0(n,0)*sx0(n,-1)*sz(n,-1)*bzg(j0(n),k0(n)-1,l(n)-1)
      bz(nn) = bz(nn) + sx0(n,1)*sx0(n,-1)*sz(n,-1)*bzg(j0(n)+1,k0(n)-1,l(n)-1)
      bz(nn) = bz(nn) + sx0(n,-1)*sx0(n,0)*sz(n,-1)*bzg(j0(n)-1,k0(n),l(n)-1)
      bz(nn) = bz(nn) + sx0(n,0)*sx0(n,0)*sz(n,-1)*bzg(j0(n),k0(n),l(n)-1)
      bz(nn) = bz(nn) + sx0(n,1)*sx0(n,0)*sz(n,-1)*bzg(j0(n)+1,k0(n),l(n)-1)
      bz(nn) = bz(nn) + sx0(n,-1)*sx0(n,1)*sz(n,-1)*bzg(j0(n)-1,k0(n)+1,l(n)-1)
      bz(nn) = bz(nn) + sx0(n,0)*sx0(n,1)*sz(n,-1)*bzg(j0(n),k0(n)+1,l(n)-1)
      bz(nn) = bz(nn) + sx0(n,1)*sx0(n,1)*sz(n,-1)*bzg(j0(n)+1,k0(n)+1,l(n)-1)
      bz(nn) = bz(nn) + sx0(n,-1)*sx0(n,-1)*sz(n,0)*bzg(j0(n)-1,k0(n)-1,l(n))
      bz(nn) = bz(nn) + sx0(n,0)*sx0(n,-1)*sz(n,0)*bzg(j0(n),k0(n)-1,l(n))
      bz(nn) = bz(nn) + sx0(n,1)*sx0(n,-1)*sz(n,0)*bzg(j0(n)+1,k0(n)-1,l(n))
      bz(nn) = bz(nn) + sx0(n,-1)*sx0(n,0)*sz(n,0)*bzg(j0(n)-1,k0(n),l(n))
      bz(nn) = bz(nn) + sx0(n,0)*sx0(n,0)*sz(n,0)*bzg(j0(n),k0(n),l(n))
      bz(nn) = bz(nn) + sx0(n,1)*sx0(n,0)*sz(n,0)*bzg(j0(n)+1,k0(n),l(n))
      bz(nn) = bz(nn) + sx0(n,-1)*sx0(n,1)*sz(n,0)*bzg(j0(n)-1,k0(n)+1,l(n))
      bz(nn) = bz(nn) + sx0(n,0)*sx0(n,1)*sz(n,0)*bzg(j0(n),k0(n)+1,l(n))
      bz(nn) = bz(nn) + sx0(n,1)*sx0(n,1)*sz(n,0)*bzg(j0(n)+1,k0(n)+1,l(n))
      ...
      bz(nn) = bz(nn) + sx0(n,0)*sx0(n,1)*sz(n,1)*bzg(j0(n),k0(n)+1,l(n)+1)
      bz(nn) = bz(nn) + sx0(n,1)*sx0(n,1)*sz(n,1)*bzg(j0(n)+1,k0(n)+1,l(n)+1)
      bz(nn) = bz(nn) + sx0(n,-1)*sx0(n,-1)*sz(n,2)*bzg(j0(n)-1,k0(n)-1,l(n)+2)
      bz(nn) = bz(nn) + sx0(n,0)*sx0(n,-1)*sz(n,2)*bzg(j0(n),k0(n)-1,l(n)+2)
      bz(nn) = bz(nn) + sx0(n,1)*sx0(n,-1)*sz(n,2)*bzg(j0(n)+1,k0(n)-1,l(n)+2)
      bz(nn) = bz(nn) + sx0(n,-1)*sx0(n,0)*sz(n,2)*bzg(j0(n)-1,k0(n),l(n)+2)
      bz(nn) = bz(nn) + sx0(n,0)*sx0(n,0)*sz(n,2)*bzg(j0(n),k0(n),l(n)+2)
      bz(nn) = bz(nn) + sx0(n,1)*sx0(n,0)*sz(n,2)*bzg(j0(n)+1,k0(n),l(n)+2)
      bz(nn) = bz(nn) + sx0(n,-1)*sx0(n,1)*sz(n,2)*bzg(j0(n)-1,k0(n)+1,l(n)+2)
      bz(nn) = bz(nn) + sx0(n,0)*sx0(n,1)*sz(n,2)*bzg(j0(n),k0(n)+1,l(n)+2)
      bz(nn) = bz(nn) + sx0(n,1)*sx0(n,1)*sz(n,2)*bzg(j0(n)+1,k0(n)+1,l(n)+2)
