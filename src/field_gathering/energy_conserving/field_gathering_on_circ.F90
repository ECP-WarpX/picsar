...
!> @param[in] frg, ftg, fzg field arrays
!
! ________________________________________________________________________________________
subroutine pxr_getf2drz_n_energy_conserving(np, xp, yp, zp, ex, ey, ez, &
                                            xmin, zmin, dx, dz, nmodes, nox, noz, &
                                            frg, frg_nguard, frg_nvalid, &
                                            ftg, ftg_nguard, ftg_nvalid, &
                                            fzg, fzg_nguard, fzg_nvalid) !#do not wrap
  ...
  INTEGER(idp), intent(IN) :: ftg_nvalid(2), fzg_nguard(2), fzg_nvalid(2)
  ...
  COMPLEX(cpx), intent(IN) :: fzg(-fzg_nguard(1):fzg_nvalid(1)+fzg_nguard(1)-1,        &
                                  -fzg_nguard(2):fzg_nvalid(2)+fzg_nguard(2)-1, 0:nmodes-1)
  REAL(num), intent(IN)    :: xmin, zmin, dx, dz

  INTEGER(idp) :: ip, j, l, ixmin, ixmax, izmin, izmax
  INTEGER(idp) :: ixmin0, ixmax0, izmin0, izmax0, jj, ll, m
  ...
  do ip = 1, np
    ...
    do ll = izmin, izmax
      do jj = ixmin, ixmax
        ex(ip) = ex(ip) + sx(jj)*sz(ll)*real(frg(j+jj,l+ll,0)*costheta - ftg(j+jj,l+ll,0)*sintheta)
        ey(ip) = ey(ip) + sx(jj)*sz(ll)*real(frg(j+jj,l+ll,0)*sintheta + ftg(j+jj,l+ll,0)*costheta)
        ez(ip) = ez(ip) + sx(jj)*sz(ll)*real(fzg(j+jj,l+ll,0))
      end do
    end do
    ...
    do m = 1, nmodes-1
      ...
      do ll = izmin, izmax
        do jj = ixmin, ixmax
          ...
          ezc = real(fzg(j+jj,l+ll,m)*xy, num)
