...
xcoefs=0._num
DO i=1, norderx/2
	xcoefs=xcoefs+wx(i)*2.0_num*SIN(kxunit*(xi*i+1_idp)*dx/xi)
ENDDO 
! - Init ycoefs
ycoefs=0._num 
DO i=1, nordery/2
	ycoefs=ycoefs+wy(i)*2.0_num*SIN(kyunit*(xi*i+1_idp)*dy/xi)
ENDDO 
! - Init zcoefs 
zcoefs=0._num
DO i=1, norderx/2
	zcoefs=zcoefs+wz(i)*2.0_num*SIN(kzunit*(xi*i+1_idp)*dz/xi)
ENDDO 

! Init kxunit_mod 
kxtemp=kxunit
kxtemp(1)=1. ! Put 1 where kx==0
kxunit_mod=kxunit*xcoefs/(kxtemp*dx)
! Init kyunit_mod 
kytemp=kyunit
kytemp(1)=1. ! Put 1 where ky==0
kyunit_mod=kyunit*ycoefs/(kytemp*dy)
! Init kzunit_mod 
kztemp=kzunit
kztemp(1)=1. ! Put 1 where kz==0
kzunit_mod=kzunit*zcoefs/(kztemp*dz)
...
! components 
CALL fast_fftw_create_plan_r2c_3d_dft(nopenmp,nx,ny,nz,ex, &
    exf,plan_r2c,INT(FFTW_MEASURE,idp),INT(FFTW_FORWARD,idp))
CALL fast_fftw_create_plan_c2r_3d_dft(nopenmp,nx,ny,nz,exf, &
...
DO i=1,p/2
	...
	IF (is_staggered) THEN 
		lognumer = LOG(16.0_num)*(1.0_num-p/2.0_num)+LOG(1.0_num*factorial(p-1_idp))*2.0_num
		logdenom = LOG(2.0_num*l-1.0_num)*2.0_num+ &
		LOG(1.0_num*factorial(p/2_idp+l-1_idp))+LOG(1.0_num*factorial(p/2_idp-l))+ &
		2.0_num*LOG(1.0_num*factorial(p/2_idp-1_idp))
	ELSE
		lognumer = LOG(1.0_num*factorial(p/2_idp))*2.0_num
		logdenom = LOG(1.0_num*factorial(p/2_idp+l))+ &
		LOG(1.0_num*factorial(p/2_idp-l))+LOG(1.0_num*l)
	ENDIF 
	w(i) = (-1.)**(l+1)*EXP(lognumer-logdenom)
END DO 
...
CALL fast_fftw3d_r2c_with_plan(nx,ny,nz,ex, exf, plan_r2c)
CALL fast_fftw3d_r2c_with_plan(nx,ny,nz,ey, eyf, plan_r2c)
CALL fast_fftw3d_r2c_with_plan(nx,ny,nz,ez, ezf, plan_r2c)
CALL fast_fftw3d_r2c_with_plan(nx,ny,nz,bx, bxf, plan_r2c)
CALL fast_fftw3d_r2c_with_plan(nx,ny,nz,by, byf, plan_r2c)
CALL fast_fftw3d_r2c_with_plan(nx,ny,nz,bz, bzf, plan_r2c)
CALL fast_fftw3d_r2c_with_plan(nx,ny,nz,jx, jxf, plan_r2c)
CALL fast_fftw3d_r2c_with_plan(nx,ny,nz,jy, jyf, plan_r2c)
CALL fast_fftw3d_r2c_with_plan(nx,ny,nz,jz, jzf, plan_r2c)
CALL fast_fftw3d_r2c_with_plan(nx,ny,nz,rhoold, rhooldf, plan_r2c)
CALL fast_fftw3d_r2c_with_plan(nx,ny,nz,rho, rhof, plan_r2c)
...
coeff_norm= SIZE(ex)
ex=ex/coeff_norm; ey=ey/coeff_norm; ez=ez/coeff_norm 
bx=bx/coeff_norm; by=by/coeff_norm; bz=bz/coeff_norm 
jx=jx/coeff_norm; jy=jy/coeff_norm; jz=jz/coeff_norm 
rho=rho/coeff_norm; rhoold=rhoold/coeff_norm; 
...
ERhomult=jj*(-EJmult/dt-1/eps0)/kmag 
ERhooldmult = jj*(coswdt/eps0+EJmult/dt)/kmag 
temp=1./(kmag*clight*eps0) ! Jmult 
BJmult=jj*(coswdt-1)*temp/clight 
