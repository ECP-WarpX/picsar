...
! ________________________________________________________________________________________
!> @brief
!> This subroutine pushes the momentum to the next step using the Boris pusher taking into account the classical Radiation Reaction (RR) force.
!
!> @author
!> Mathieu Lobet
!> Henri Vincenti
...
!
! ________________________________________________________________________________________
SUBROUTINE pxr_boris_push_rr_u_3d(np, uxp, uyp, uzp, gaminv, ex, ey, ez, bx, by, bz, q,  &
  m, dt)
  USE constants
  IMPLICIT NONE
  ! Input/Output parameters
